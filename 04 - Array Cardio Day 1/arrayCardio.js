...
console.log(inventors.reduce((acc, i) => acc += parseInt(i.passed - i.year, 10), 0));
...
// Array.from(document.querySelectorAll('.mw-category a')).filter(b => b.title.includes('de'))

// 7. sort Exercise
// Sort the people alphabetically by last name
const orderedPeople = people.sort((prev, next) => {

    const [prevLast, prevFirst] = prev.split(',');
    const [nextLast, nextFirst] = next.split(',');
    return prevLast > nextLast ? 1 : 0;
})
